...
function generatePortfolioHTML(projects, containerId) {
    ...
    projects.forEach((project, index) => {
        ...
        const categories = project.categories && Array.isArray(project.categories) ? project.categories : ['web'];
        ...
        html += '<div class="item" data-groups="' + dataGroupsStr + '">' + '<div class="portfolio-card" data-project-id="' + escapeHtml(project.id) + '" onclick="showProjectDetail(\'' + escapedProjectId + '\')">' + '<div class="portfolio-image">' + '<img alt="' + escapeHtml(project.title) + '" src="' + escapeHtml(project.image) + '">' + '</div>' + '<div class="portfolio-content">' + '<h4>' + escapeHtml(project.title) + '</h4>' + '<p>' + escapeHtml(project.description) + '</p>' + techTagsHTML + '</div></div></div>';
    });
    if (containerId) {
